...
const settings = {
    ...
    REDIRECT_AFTER_ACTION: false, //Redirect to getConnectedSockets JSON view after action (true) or acknowledge action (false)
    plugInfo : [
...
const requestHandler = (request, response) => {
    ...
    if (q.uid != null) {
      if (request.method == "POST") {
        ...
        console.log("Got the following POST:");
        var data = '';
      
	request.on('data', function(chunk) {
	action = chunk.toString();
        result = orvbio.setSocketMode(q.uid,action);

        console.log("Setting socket to uid="+q.uid+" mode="+action)

        });
      } else {
        //List status!
        //No longer toggle - now we list the status!
        let socketState = [];
        socketStateVal = orvbio.getSocketMode(q.uid);
        console.log("Getting socket status for uid="+q.uid+" state="+socketStateVal);
        if (socketStateVal===0) {
          //It is on!
          result = '{"is_active": "true"}';
        } else if (socketStateVal===false) {
          result = '{"is_active": "unknown"}';
        } else {
          result = '{"is_active": "false"}';
        }
        //result = JSON.stringify(socketState);
